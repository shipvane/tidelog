...
const MARGINAL_CLEARANCE_THRESHOLD_M = 0.3;
...
/**
 * Find the peak (maximum) tide height within a time window [startMs, endMs].
 * Considers both table entry points and local maxima on rising segments.
 */
function peakHeightInWindow(points, startMs, endMs) {
  let peak = -Infinity;

  for (let i = 0; i < points.length; i += 1) {
    const p = points[i];
    // Include any table point within the window
    if (p.t >= startMs && p.t <= endMs) {
      peak = Math.max(peak, p.heightM);
    }

    // For segments crossing the window, check for peaks
    if (i < points.length - 1) {
      const a = points[i];
      const b = points[i + 1];

      // If the segment overlaps with the window
      if (a.t < endMs && b.t > startMs) {
        // Peak of a segment is at one of its endpoints or where it starts/ends the window
        peak = Math.max(peak, a.heightM, b.heightM);
      }
    }
  }
 ...
 * - peakClearanceM: how much the peak tide height exceeds the required height
 * - marginal: true if peakClearanceM < 0.3m (risky; consider waiting for springs)
 */
function safeWindows(entries, constraints) {
  ...
  return raw
    ...
    .map((w) => {
      const peak = peakHeightInWindow(points, w.start, w.end);
      const clearance = peak - required;
      return {
        start: new Date(w.start).toISOString(),
        end: new Date(w.end).toISOString(),
        durationMinutes: Math.round((w.end - w.start) / 60000),
        peakClearanceM: clearance,
        marginal: clearance < MARGINAL_CLEARANCE_THRESHOLD_M,
      };
    });
}
