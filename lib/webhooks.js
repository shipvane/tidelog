...
/**
 * The HTTP transport used for deliveries. This is a seam, not a behaviour
 * change: the default is the global `fetch`, called exactly as before, so the
 * product path is untouched. Tests swap it out via `setTransport` so the suite
 * makes no real outbound request. The 10s timeout and 5s retry below are
 * product behaviour and are deliberately left alone.
 * @type {(url: string, options: object) => Promise<{ok: boolean, status: number}>}
 */
let transport = (url, options) => fetch(url, options);

/**
 * Replace the delivery transport (tests only). Call with no argument to
 * restore the default global-`fetch` transport.
 * @param {?((url: string, options: object) => Promise<{ok: boolean, status: number}>)} [fn]
 */
function setTransport(fn) {
  transport = fn || ((url, options) => fetch(url, options));
}
...
module.exports = { fireEvent, deliverToSubscription, attemptDelivery, setTransport };
