...
/**
 * Transport seam. `null` means "use the global `fetch`", which is the only
 * path production ever takes — the call below is byte-for-byte identical to a
 * bare `fetch(url, { ... })`. Tests set an override with `__setFetch` so no
 * real outbound request is ever made from the suite (SVE-151); the default
 * behaviour, the 10 s timeout and the single 5 s retry are untouched.
 * @type {typeof fetch | null}
 */
let transportOverride = null;

/** Test seam: route deliveries through `fn` instead of the global `fetch`. */
function __setFetch(fn) {
  transportOverride = fn;
}

/** Test seam: restore the default (global `fetch`) transport. */
...
async function attemptDelivery(url, payload) {
  ...
  const doFetch = transportOverride || fetch;
  try {
...
module.exports = { fireEvent, deliverToSubscription, attemptDelivery, __setFetch, __resetFetch };
