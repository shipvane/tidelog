 ...
 *
 * Berths are `{ id, name, lengthM, depthM, outOfService }` (with optional outOfService
 * boolean); assignments are `{ berthId, arrivalId, vesselName, from, to }` with ISO timestamps.
 */
...
function findBerth(berths, assignments, vessel, { from, to }) {
  ...
  const candidates = berths.filter(
    (berth) =>
      !berth.outOfService &&
      fitsBerth(berth, vessel).fits &&
      !hasConflict(assignments, berth.id, from, to)
  );
