...
- [ ] PWA (2/3): service worker and offline app shell — tracked as **SVD-12**
  ...
  - **Decide the staleness trade-off explicitly, and write the decision down.** #39 is pure cache-first with no revalidation (`caches.match(...).then(r => r || fetch(request))`) and deliberately omits `skipWaiting`/`clients.claim`, so a new SW only activates once EVERY tab closes. The reasoning it gives (no jarring mid-session updates) is sound in isolation, but **on this repo a push to `main` IS the deploy** and there is no staging — so a shipped change can silently never reach a returning visitor. Prefer stale-while-revalidate for the shell so the cache refreshes in the background, or keep cache-first and state plainly in the PR that you accepted the trade.
